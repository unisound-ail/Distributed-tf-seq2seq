...
#sys.path.append("/home/xuerq/github/Distributed-tf-seq2seq/")
...
def train():
  ...
  #with tf.Session() as sess:
    ...
    encoder_inputs = [[0 for x in range(50)] for i in xrange(FLAGS.num_gpus)]
    decoder_inputs = [[0 for x in range(50)] for i in xrange(FLAGS.num_gpus)]
    target_weights = [[0 for x in range(50)] for i in xrange(FLAGS.num_gpus)]
    tower_grads = []
    step_losses = []
    ...
    updates = opt.apply_gradients(
          zip(average_grads, params), global_step= global_step)
    ...
    while True:
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        print ("global step %d learning rate %.4f step-time %.2f sample-per-sec %.2f perplexity "
               "%.2f" % ( global_step.eval(), model_list[0].learning_rate.eval(),
                         step_time,
                         FLAGS.num_gpus*FLAGS.batch_size/step_time, 
                         perplexity))
        ...
        for bucket_id in xrange(len(_buckets)):
          if len(dev_set[bucket_id]) == 0:
            ...
            continue
          encoder_inputs, decoder_inputs, target_weights = model_list[0].get_batch(
              dev_set, bucket_id)
          _, eval_loss, _ = model_list[0].step(sess, encoder_inputs, decoder_inputs,
